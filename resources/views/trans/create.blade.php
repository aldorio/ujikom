...
<body>
    <div class="container">
        ...
        <div class="main-content">
            ...
            <div class="card">
                ...
                <form method="POST" action="{{ route('trans.store')}}" id="transactionForm">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <select name="id_customer" id="customerName" class="form-control">
                            ...
                            @foreach ($customers as $customer)
                                <option data-phone="{{ $customer->phone }}" data-address="{{ $customer->addres }}" value="{{ $customer->id }}">{{ $customer->name }}</option>
                            @endforeach
                        </select>
                    </div>
    ...
    <script>
       ...
       function addToCart() {
    const selectedService = document.querySelector('#serviceType');

    // Ambil nilai dari elemen <option> yang terpilih.
    // .value akan mengambil ID atau nilai yang Anda tetapkan.
    // .text akan mengambil nama layanan yang terlihat oleh pengguna.
    const serviceType = selectedService.options[selectedService.selectedIndex].text;

    // Ambil harga layanan dari atribut data-price yang ada di elemen <option>
    const priceService = selectedService.options[selectedService.selectedIndex].dataset.price;

    const weightValue = document.getElementById('serviceWeight').value;
    const weight = parseFloat(weightValue); // Gunakan parseFloat untuk angka desimal

    const notes = document.getElementById('notes').value;

    // Lakukan validasi: cek apakah layanan sudah dipilih dan berat valid
    if (!selectedService.value || isNaN(weight) || weight <= 0) {
        alert('Mohon pilih layanan dan masukkan berat yang valid.');
        return;
    }

    // Hitung subtotal. Pastikan priceService diubah ke number.
    const subtotal = parseFloat(priceService) * weight;

    // Buat objek item yang akan dimasukkan ke keranjang
    const item = {
        id: Date.now(),
        service: serviceType,
        weight: weight,
    ...
    // Tambahkan item ke array 'cart' yang sudah dideklarasikan di luar fungsi
    cart.push(item);

    // Perbarui tampilan keranjang dan total pembayaran
    updateCartDisplay();

    // Kosongkan form setelah item ditambahkan
    document.getElementById('serviceType').value = '';
    document.getElementById('serviceWeight').value = '';
    document.getElementById('notes').value = '';
}
